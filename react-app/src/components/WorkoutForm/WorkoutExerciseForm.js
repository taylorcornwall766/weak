...
function WorkoutExerciseForm({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!weight) {
      setErrors({
        ...errors,
        weight:
          "Please provide a valid amount of weight, if its a body weight exercise put your body weight.",
      });
      errors = true;
    }
    if (!exerciseId) {
    ...
    if (errors) {
      ...
      const data = await dispatch(
        postWorkoutExerciseThunk(
          { exercise_id: exerciseId, reps: reps, weight: weight },
          workoutId
        )
      );
  ...
  return (
    <form
      className={`edit-routine-form editableCard ${
        clicked ? "is-clicked" : "isNot-clicked"
      }`}
      onClick={() => {
        setReadOnly(false);
      }}
